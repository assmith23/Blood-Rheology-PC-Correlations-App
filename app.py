...
def calculate_correlation_stats(x_data, y_data):
    """
    Calculate correlation coefficient, p-value, and regression statistics.
    This mirrors your existing statistical analysis approach.
    """
...
def create_correlation_plot(pc_idx, rheo_idx):
    """
    Create an interactive correlation plot matching your original design.
    This function encapsulates all the plotting logic from your code.
    """
    # Extract data
    x_data = X_pca[:, pc_idx]
    ...
    fig.update_layout(
        title=dict(
            text=f'PC{pc_idx+1} vs {rheology_params[rheo_idx]} Correlation (r = {stats_result["correlation"]:.3f})',
            x=0.5,
            font=dict(size=16)
        ),
        xaxis_title=f'PC{pc_idx+1} ({pc_variance:.1f}% variance)',
        yaxis_title=rheology_params[rheo_idx],
        width=800,
        height=600,
        showlegend=True,
